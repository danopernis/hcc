...
#include "hcc/util/index_table.h"
...
struct globals_table : util::index_table<globals_table, std::string, global> {
...
struct subroutine_ir {
    ...
    void for_each_bb_in_domtree_preorder(F&& f)
    {
        ...
        util::depth_first_search dfs(dominance->tree.successors(), dominance->root);
...
private:
    ...
    label exit_node_;
    label entry_node_;
    std::unique_ptr<util::graph_dominance> reverse_dominance;
    std::unique_ptr<util::graph_dominance> dominance;
...
struct unit {
    ...
    void translate_to_asm(hcc::assembler::program&);
    void translate_from_jack(const hcc::jack::Class&);
};
