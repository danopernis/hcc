...
void do_spill(subroutine& s, const reg& r)
{
    ...
    s.for_each_bb([&](basic_block& bb) {
        for (auto i = bb.instructions.begin(), e = bb.instructions.end(); i != e;) {
            ...
            const argument spill_second
                = argument(s.regs.put(x + "_" + std::to_string(++spill_counter)));
            ...
            if (did_spill_load) {
                bb.instructions.insert(
                    i, hcc::ssa::instruction(instruction_type::LOAD, {spill_second, spill_first}));
            }
            if (did_spill_store) {
                if (instruction.type == instruction_type::MOV
                    && instruction.arguments[0] == spill_second) {
                    instruction.type = instruction_type::STORE;
                    instruction.arguments[0] = spill_first;
                    ++i;
                } else {
                    ++i;
                    bb.instructions.insert(i, hcc::ssa::instruction(instruction_type::STORE,
                                                                    {spill_first, spill_second}));
                }
            } else {
                ++i;
            }
