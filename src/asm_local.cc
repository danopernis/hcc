...
struct constant_propagation {
    void operator()(hcc::asm_instruction& cmd)
    {
        auto handle_ainstr = [&] (const constant_value& new_A) {
            if (r.A == new_A) {
                cmd.type = hcc::asm_instruction_type::VERBATIM;
                cmd.instr = COMPUTE | RESERVED | COMP_ZERO;
            } else {
                r.A = new_A;
...
struct dead_code_elimination {
    void operator()(hcc::asm_instruction& command)
    {
        ...
        if (live) {
            ...
            command.type = hcc::asm_instruction_type::VERBATIM;
            command.instr = COMPUTE | RESERVED | COMP_ZERO;
        }
    }
...
{
    return i.type == hcc::asm_instruction_type::VERBATIM &&
        i.instr == (COMPUTE | RESERVED | COMP_ZERO);
}
